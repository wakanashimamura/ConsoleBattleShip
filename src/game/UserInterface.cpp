...
void UserInterface::UpdateMainMenu() {
  static const std::vector<std::string> MAIN_MENU_ITEMS =
      {"Start Game", "Statistics", "Tutorials", "Settings", "Exit"};
...
void UserInterface::HandleInputMainMenu(Game& game) {
  static const std::unordered_map<MainMenuItem, std::function<void()>> menuActions = {
      ...
      {MainMenuItem::Exit, [&game]() {
         game.SetIsLoadGame(false);
         game.SaveData();
       }}
  };
...
void UserInterface::RenderStatistics(
    const Player& player1, const Player& player2, const Player& player3
) {
  ...
  for (;;) {
    ...
    switch (key) {
      ...
      case InputManager::Key::D:
        currentTutorialPageIndex < TUTORIAL_PAGES_COUNT - 1 ? currentTutorialPageIndex++
...
void UserInterface::UpdateSettings() {
  static const std::vector<std::string> SETTINGS_MENU_ITEMS = {"Name Change", "Game Mode", "Exit"};
...
void UserInterface::HandleInputSettings() {
  static const std::unordered_map<SettingsMenuItem, std::function<void()>> menuActions = {
      {SettingsMenuItem::NameChange,
       [this]() { currentMenuWindow = MenuWindow::SettingsNameChange; }},
      {SettingsMenuItem::GameMode, [this]() { currentMenuWindow = MenuWindow::SettingsGameMode; }},
      {SettingsMenuItem::Exit, [this]() {
         currentMenuWindow = MenuWindow::MainMenu;
         RestoreSettingsSelectedItem();
       }}
  };
...
void UserInterface::UpdateNameChangeSettings() {
  ...
  UpdateContent(
      settingsNameChangeContent,
      SETTINGS_NAME_CHANGE_MENU_ITEMS,
      settingsNameChangeSelectedItem,
      SETTINGS_NAME_CHANGE_FILE_NAME
  );
}
...
void UserInterface::HandleInputNameChangeSettings(Player& player1, Player& player2) {
  static const std::unordered_map<SettingsNameChangeMenuItem, std::function<void()>> menuActions = {
      {SettingsNameChangeMenuItem::Player1,
       [this, &player1]() {
         ChangName(player1);
         FileManager::WritePlayerInBinaryFile(Game::PLAYER1_FILE_NAME, player1);
       }},
      {SettingsNameChangeMenuItem::Player2,
       [this, &player2]() {
         ChangName(player2);
         FileManager::WritePlayerInBinaryFile(Game::PLAYER2_FILE_NAME, player2);
       }},
      {SettingsNameChangeMenuItem::Exit, [this]() {
         currentMenuWindow = MenuWindow::Settings;
         RestoreSettingsNameChangeSelectedItem();
       }}
  };
  ...
  HandleKeyboardInput(
      settingsNameChangeSelectedItem,
      SettingsNameChangeMenuItem::Player1,
      SettingsNameChangeMenuItem::Exit,
      menuActions,
      [this]() {
        currentMenuWindow = MenuWindow::Settings;
        RestoreSettingsNameChangeSelectedItem();
...
void UserInterface::UpdateGameModeSettings(const Game& game) {
  ...
  UpdateContent(
      settingsGameModeContent,
      SETTINGS_GAME_MODE_MENU_ITEMS,
      settingsGameModeSelectedItem,
      SETTINGS_GAME_MODE_FILE_NAME
  );

  if (size_t arrowPosition = settingsGameModeContent.find("X");
  ...
  if (selectedItemPosition == std::string::npos) {
    throw GameException(
        "File " + SETTINGS_GAME_MODE_FILE_NAME +
        ", or array SETTINGS_GAME_MODE_MENU_ITEMS is incorrect!"
    );
  }
}
...
void UserInterface::HandleInputGameModeSettings(Game& game) {
  static const std::unordered_map<SettingsGameModeMenuItem, std::function<void()>> menuActions = {
      {SettingsGameModeMenuItem::PlayerVsPlayer,
       [this, &game]() {
         game.SetGameMode(GameMode::PlayerVsPlayer);
         FileManager::WriteGameModeInBinaryFile(Game::GAME_MODE_FILE_NAME, game.GetGameMode());
       }},
      {SettingsGameModeMenuItem::PlayerVsBot,
       [this, &game]() {
         game.SetGameMode(GameMode::PlayerVsBot);
         FileManager::WriteGameModeInBinaryFile(Game::GAME_MODE_FILE_NAME, game.GetGameMode());
       }},
      {SettingsGameModeMenuItem::Exit, [this]() {
         currentMenuWindow = MenuWindow::Settings;
         RestoreSettingsGameModeSelectedItem();
       }}
  };
...
void UserInterface::UpdateShipsSetupMenu() {
  ...
  UpdateContent(
      shipsSetupMenuContent,
      SHIPS_SETUP_MENU_ITEMS,
      shipsSetupMenuSelectedItem,
      SHIPS_SETUP_MENU_FILE_NAME
  );
}
    ...
    if (std::string code = Content.substr(index, 2); code == ">>") {
      ...
      if (index + 21 < Content.size()) {
        ...
        Console::SetCursorPosition(
            Console::GetCursorPosition().first + 21,
            Console::GetCursorPosition().second
        );
      } else {
  ...
  for (;;) {
    ...
    switch (key) {
      ...
      case InputManager::Key::W:
        ChangeMenuItemUp(
            shipsSetupMenuSelectedItem,
            ShipsSetupMenuItem::BattleShip,
            ShipsSetupMenuItem::Exit
        );
...
void UserInterface::RenderGameBoard(
  ...
  for (int i = 0; i < gameBoard.size(); ++i) {
    ...
    for (int j = 0; j < gameBoard[0].size(); ++j) {
      if (gameBoard[i][j] == static_cast<char>(Board::BoardSymbol::EmptySymbol))
        ...
        Console::ChangeColor(color.missColor);
      else if (j >= start.x && j <= end.x && i >= start.y && i <= end.y)
...
void UserInterface::RenderGameBoard(
    const Player& player1, bool RenderMode1, const Player& player2, bool RenderMode2
) {
  const Board::GameBoard& gameBoard1 =
      RenderMode1 ? player1.GetBoard().GetGameBoard() : player1.GetBoard().GetHiddenGameBoard();
  const Board::GameBoard& gameBoard2 =
      RenderMode2 ? player2.GetBoard().GetGameBoard() : player2.GetBoard().GetHiddenGameBoard();
...
void UserInterface::SetApplicationSettings() {
  Console::SetTitle("ConsoleBattleShip");
...
void UserInterface::RenderBoardHeader(
    Vector2D& boardPosition, const std::string& playerName, const Board::GameBoard& gameBoard
) {
  Console::ChangeColor(Console::Color::White);
...
void UserInterface::UpdateContent(
    std::string& content,
    const std::vector<std::string>& menuItems,
    const auto& menuSelectedItem,
  ...
  if (selectedItemPosition = content.find(menuItems[static_cast<int>(menuSelectedItem)]);
      selectedItemPosition != std::string::npos) {
...
void UserInterface::ChangName(Player& player) {
  ...
  for (;;) {
    ...
    if (((inputChar >= '!' && inputChar <= '9') || (inputChar >= '?' && inputChar <= 'z')) &&
        index < Player::MAX_USERNAME_LENGTH) {
...
bool UserInterface::IsColorCode(const std::string& content, const size_t& index) const {
  ...
  return (index + 2 < content.size() && content[index] == 'C' || content[index] == 'L') &&
         (ColorCode.find(content.substr(index, 2)) != ColorCode.cend());
}