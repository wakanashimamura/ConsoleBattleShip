...
class UserInterface {
 public:
  ...
  void RestoreShipsSetupMenuSelectedItem() {
    shipsSetupMenuSelectedItem = ShipsSetupMenuItem::BattleShip;
  }
  void RestoreSettingsSelectedItem() { settingsSelectedItem = SettingsMenuItem::NameChange; }
  void RestoreSettingsNameChangeSelectedItem() {
    settingsNameChangeSelectedItem = SettingsNameChangeMenuItem::Player1;
  }
  void RestoreSettingsGameModeSelectedItem() {
    settingsGameModeSelectedItem = SettingsGameModeMenuItem::PlayerVsPlayer;
  }
 ...
 private:
  ...
  void HandleKeyboardInput(
      _Ty& MenuSelectedItem,
      _Ty firstItem,
      _Ty lastItem,
      const std::unordered_map<_Ty, std::function<void()>>& menuItemActions,
  ...
  // - Logo -
  const std::string LOGO_FILE_NAME = "ui/Logo.txt";
  std::string logoContent;

  // - Main Menu -
  const std::string MAIN_MENU_FILE_NAME = "ui/MainMenu.txt";
  std::string mainMenuContent;
  MainMenuItem mainMenuSelectedItem = MainMenuItem::StartGame;

  // - Ships Setup Menu -
  const std::string SHIPS_SETUP_MENU_FILE_NAME = "ui/ShipsSetupMenu.txt";
  std::string shipsSetupMenuContent;
  ShipsSetupMenuItem shipsSetupMenuSelectedItem = ShipsSetupMenuItem::BattleShip;

  // - Battle -
  const std::string BATTLE_FILE_NAME = "ui/Battle.txt";
  std::string battleContent;

  // - Statistics -
  const std::string STATISTICS_FILE_NAME = "ui/Statistics.txt";
  ...
  const std::string TUTORIAL_PAGES_NAMES[TUTORIAL_PAGES_COUNT] = {
      "ui/TutorialPage1.txt",
      "ui/TutorialPage2.txt",
      "ui/TutorialPage3.txt",
      "ui/TutorialPage4.txt",
  };
  ...
  // - Settings -
  const std::string SETTINGS_FILE_NAME = "ui/Settings.txt";
  std::string settingsContent;
  SettingsMenuItem settingsSelectedItem = SettingsMenuItem::NameChange;

  // - Settings Name Change -
  const std::string SETTINGS_NAME_CHANGE_FILE_NAME = "ui/SettingsNameChange.txt";
  ...
  // - Name Change -
  const std::string NAME_CHANGE_FILE_NAME = "ui/NameChange.txt";
  std::string nameChangeContent;

  // - Settings Game Mode -
  const std::string SETTINGS_GAME_MODE_FILE_NAME = "ui/SettingsGameMode.txt";
...
template <std::equality_comparable _Ty>
void UserInterface::HandleKeyboardInput(
    _Ty& MenuSelectedItem,
    _Ty firstItem,
    _Ty lastItem,
    const std::unordered_map<_Ty, std::function<void()>>& menuItemActions,
