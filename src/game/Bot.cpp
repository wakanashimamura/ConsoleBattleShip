...
Bot::Bot()
    : Player({56, 9}, "Nami") {
  shipSearchState.direction = static_cast<Direction>(
      Random::GenerateInt(static_cast<int>(Direction::Up), static_cast<int>(Direction::Down))
  );
}
...
std::pair<ShotStatus, GameAction> Bot::Shot(Player& target) {
  ...
  if (shotRes == ShotStatus::Hit) {
    if (shipSearchState.hitDetected && !shipSearchState.isDirectionFound) {
      ...
      (shotRes == ShotStatus::Miss || shotRes == ShotStatus::NoneShot) &&
      shipSearchState.hitDetected
  ) {
...
void Bot::ReverseDirection() {
  switch (shipSearchState.direction) {
    case Direction::Up:
      shipSearchState.direction = Direction::Down;
      break;
    case Direction::Left:
      shipSearchState.direction = Direction::Right;
      break;
    case Direction::Right:
      shipSearchState.direction = Direction::Left;
      break;
    case Direction::Down:
      shipSearchState.direction = Direction::Up;
      break;
    default:
