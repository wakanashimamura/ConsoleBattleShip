...
bool Board::SetupPlayerShips(const std::string& playerName) {
  ...
  for (;;) {
    ...
    userInterface
        .RenderGameBoard(GetGameBoard(), symbolColors, playerName, defaultPositionGameBoard);
    ...
    if (action.second == true) {
      switch (action.first) {
        ...
        default:
          throw GameException(
              "Hasn't processed action for ShipsSetupMenuItem(" +
              std::to_string(static_cast<int>(action.first)) + ")"
          );
      }
    }
  }
...
void Board::MarkShipAsDestroyed() {
  for (const auto& [id, ship] : ships) {
    ...
    for (size_t i = surroundingArea.first.y; i <= surroundingArea.second.y && i < gameBoard.size();
         ++i) {
      for (size_t j = surroundingArea.first.x;
           j <= surroundingArea.second.x && j < gameBoard.size();
           ++j) {
...
void Board::SetupShip(Ship& ship, const std::string& playerName, bool editMode) {
  static constexpr Color colorBoard = {
      Console::Color::Cyan,
      Console::Color::Magenta,
      Console::Color::LightMagenta,
      Console::Color::Red
  };
  UserInterface& userInterface = UserInterface::GetObjectWindow();
  Ship tempShip(ship);
  ...
  for (;;) {
    Console::Color ColorShip =
        ValidateShipPosition(ship) ? Console::Color::Green : Console::Color::Red;
...
std::pair<Vector2D, Vector2D> Board::GetSurroundingArea(const Ship& ship) const {
  std::pair<Vector2D, Vector2D> surroundingArea{
      ship.GetPosition().front(),
      ship.GetPosition().back()
  };

  if (ship.GetOrientation() == Ship::Direction::Horizontal) {
    surroundingArea.first.x =
        surroundingArea.first.x == 0 ? surroundingArea.first.x : --surroundingArea.first.x;
    surroundingArea.first.y =
        surroundingArea.first.y == 0 ? surroundingArea.first.y : --surroundingArea.first.y;

    surroundingArea.second.x = surroundingArea.second.x == BOARD_SIZE - 1
                                   ? surroundingArea.second.x
                                   : ++surroundingArea.second.x;
    surroundingArea.second.y = surroundingArea.second.y == BOARD_SIZE - 1
                                   ? surroundingArea.second.y
                                   : ++surroundingArea.second.y;
  } else {
    surroundingArea.first.x = surroundingArea.first.x == BOARD_SIZE - 1 ? surroundingArea.first.x
                                                                        : ++surroundingArea.first.x;
    surroundingArea.first.y = surroundingArea.first.y == BOARD_SIZE - 1 ? surroundingArea.first.y
                                                                        : ++surroundingArea.first.y;

    surroundingArea.second.x =
        surroundingArea.second.x == 0 ? surroundingArea.second.x : --surroundingArea.second.x;
    surroundingArea.second.y =
        surroundingArea.second.y == 0 ? surroundingArea.second.y : --surroundingArea.second.y;
